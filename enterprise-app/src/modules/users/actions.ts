...
export async function createUser(userData: CreateUserRequest, database: any, events: any): Promise<User> {
  const users = database.users || [];
  const newUser: User = {
...
export async function updateUser(id: number, updateData: UpdateUserRequest, database: any, events: any): Promise<User | null> {
  const users = database.users || [];
